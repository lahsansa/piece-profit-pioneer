...
import { Copy, AlertTriangle, Check, Upload, Send, Store } from "lucide-react";
...
const TopupBalance = () => {
  ...
  const [hasPack, setHasPack] = useState<boolean | null>(null); // null = loading
  ...
  const loadData = async () => {
    ...
    const { data: store } = await supabase.from("user_stores")
      .select("store_level, total_topup").eq("user_id", user.id).maybeSingle();

    // Has pack = has paid before OR came from StoreLevels with amount param
    const fromStoreLevels = !!searchParams.get("amount");
    const paid = store && Number(store.total_topup) > 0;
    setHasPack(fromStoreLevels || paid || false);
  ...
  const handleSubmit = async () => {
    if (hasPending) { toast.error("⏳ لديك طلب شحن قيد المراجعة — انتظر حتى يتم الرد عليه"); return; }
    ...
    try {
      ...
      if (existing && existing.length > 0) {
        toast.error("⏳ لديك طلب شحن قيد المراجعة");
      ...
      const upgradeTo = searchParams.get("to");
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-6">
          ...
          {planName && (
            <div className="mt-2 inline-block bg-emerald-100 text-emerald-700 text-sm font-bold px-4 py-1.5 rounded-full">
              🚀 {searchParams.get("upgrade") ? "ترقية إلى" : "تفعيل"} {planName}
            </div>
          )}
        </motion.div>
          ...
          <div className="space-y-4">
            ...
            {!hasPack && (
              <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
                className="bg-emerald-50 border-2 border-emerald-400 rounded-2xl p-4 text-center space-y-3">
                <div className="w-12 h-12 rounded-2xl bg-emerald-100 flex items-center justify-center mx-auto">
                  <Store className="w-6 h-6 text-emerald-600" />
                </div>
                <p className="text-emerald-800 font-bold text-sm">⚠️ خاصك تختار باقة أولاً</p>
                <p className="text-emerald-600 text-xs">اختر الباقة المناسبة ليك وبعدها ارجع هنا للشحن</p>
                <Button
                  className="w-full h-10 font-bold bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl"
                  onClick={() => navigate("/store-levels")}
                >
                  🚀 اختر باقتك الآن
                </Button>
              </motion.div>
            ...
            <Card className={hasPending || !hasPack ? "opacity-40 pointer-events-none" : ""}>
              <CardContent className="p-4 space-y-3">
            ...
            <div className="text-center">
              <button onClick={() => navigate("/store-levels")}
                className="text-xs text-muted-foreground underline hover:text-primary">
                تغيير الباقة / الترقية
              </button>
            </div>
          </div>
